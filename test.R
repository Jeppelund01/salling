...
zips <- c("3400", "3450", "3600", "4600")

# Lille hjælpefunktion: lav NULL om til NA
...
get_foodwaste_zip <- function(zip) {
  ...
  if (length(stores_list) == 0) {
    message("Ingen butikker/tilbud for: ", zip)
    return(NULL)
  }
  ...
  # Hvis der ikke kom nogen rækker ud, returnér NULL
  if (length(rows) == 0) {
    message("Ingen tilbud fundet for: ", zip)
    return(NULL)
  }
...
if (length(all_data_list) == 0) {
  warning("Der blev ikke hentet nogen data for nogen postnumre.")
  foodwaste_data <- data.frame()
} else {
...
message("Antal rækker i foodwaste_data: ", nrow(foodwaste_data))
print(head(foodwaste_data))

####################################################
# Gem resultat som CSV-fil i din projektmappe
####################################################

if (nrow(foodwaste_data) > 0) {
  write.csv(
    foodwaste_data,
    "salling_foodwaste_4_postnumre.csv",
    row.names = FALSE
  )
  message("CSV gemt som: salling_foodwaste_4_postnumre.csv")
} else {
  message("Ingen data at gemme i CSV.")
}
